( function( $ ) {
	$( window ).load( function() {
		...
		$( 'div[id^="wr-' + wr_nitro_customize_radio_image.type + '-"]' ).each( function() {
			var self = $( this );

			// Get the control ID.
			self.id = self.attr( 'id' ).replace( 'wr-' + wr_nitro_customize_radio_image.type + '-', '' );

			// Init radio images.
			self.find( '.wr-radio-image' ).click( function() {
