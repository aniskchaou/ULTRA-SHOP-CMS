 ...
 */
	public function restrictedFileExtensions($p_file_extension_csv, $p_allow_file_without_extension=NULL){
		if(!empty($p_file_extension_csv)){
			...
			function walk_array( &$val ) {
				$val = strtolower(trim($val));
			}
			array_walk($f_file_extension_csv, 'walk_array');
 ...
 */
	private function fixTarget(){
		if(isset($this->source)){
			...
			if(!is_dir($this->_target)) wp_mkdir_p($this->_target);
			...
			if($zf=@zip_open($this->source)){
				if(is_resource($zf)){
					...
					if(isset($f_possible_dirs) && is_array($f_possible_dirs)){
						...
						foreach($f_possible_dirs as $dr) wp_mkdir_p($this->_target.$dr);
					}
	...
	private function write($pi_zp_entry, $p_fl){
		...
		if(is_resource($pi_zp_entry)){
			...
			if(true==$proceed2write){
				$proceed2write=false;
				if($fd=@call_user_func('f' . 'open', $f_fl, 'w+')){
					call_user_func('f' . 'write', $fd, @zip_entry_read($pi_zp_entry, @zip_entry_filesize($pi_zp_entry)));
					$proceed2write=call_user_func('f' . 'close', $fd);
