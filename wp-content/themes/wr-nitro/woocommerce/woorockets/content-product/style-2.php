		...
		<div class="product__action icon_color pa fc jcc">
			<?php
				...
				if ( $wr_nitro_options['wc_general_quickview'] ) {
					echo '<a class="product__btn bts-40 btn-quickview body_bg dib pr hover-primary" href="#0" data-prod="' . esc_attr( $post->ID ) . '"><i class="nitro-icon-' . esc_attr( $wr_icons ) . '-quickview"></i><span class="tooltip ab">' . esc_html__( 'Quick View', 'wr-nitro' ) . '</span></a>';
				}
				...
				if ( $wr_nitro_options['wc_buynow_btn'] && $product->is_purchasable() && $product->is_type( 'simple' ) && ! $wr_catalog_mode && $product->is_in_stock() && ! WR_Nitro_Helper::check_gravityforms( $post->ID ) && ! WR_Nitro_Helper::yith_wc_product_add_ons( $post->ID ) && ! WR_Nitro_Helper::wc_measurement_price_calculator( $post->ID ) && $booster_pop != 'yes' ) {
					echo '<a class="product__btn bts-40 body_bg db dib pr btn-buynow hover-primary" href="#" data-product-id="' . get_the_ID() . '"><i class="nitro-icon-' . esc_attr( $wr_icons ) . '-quickbuy"></i><span class="tooltip ab">' . esc_html__( 'Buy Now', 'wr-nitro' ) . '</span></a>';
				}
				...
				if ( class_exists( 'YITH_WOOCOMPARE' ) && $wr_show_compare && ! $wr_catalog_mode ) {
					echo '
						<div class="product__compare">
							<a class="product__btn bts-40 body_bg mg0 dib" href="#"><i class="nitro-icon-' . esc_attr( $wr_icons ) . '-compare"></i><span class="tooltip ab">' . esc_html__( 'Compare', 'wr-nitro' ) . '</span></a>
