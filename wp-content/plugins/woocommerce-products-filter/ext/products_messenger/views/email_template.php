...
<div class="woof_search_terms"><p>
	<?php _e('Terms: ', 'woocommerce-products-filter') ?>
	<?php echo $search_terms ?>
    </p></div>
<?php if ($last_email) { ?>
    <div class="last_email"><?php _e('Attention! This is the last email. If you want to continue get such emails -> Go by next link and subscribe again', 'woocommerce-products-filter') ?>
        - <a href="<?php echo $subscr['link'] . "&orderby=date&order=DESC" ?>"><?php _e('Subscribe ', 'woocommerce-products-filter'); ?></a>
    </div>
<?php } ?>
...
<ul class="products woof_mail" >
	...
	if ($i < count($products->posts)) {
		...
		<p style="text-align: center;font-size: 18px;" ><a style="padding: 4px;border: 2px solid #557DA1; text-decoration: none;" href="<?php echo $subscr['link'] . "&orderby=date&order=DESC" ?>"><?php _e('See more new products...', 'woocommerce-products-filter') ?></a></p>
	    </div> 
	<?php
	}
    } else {
